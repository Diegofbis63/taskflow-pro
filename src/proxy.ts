...
// This is a temporary proxy to replace deprecated middleware
export default createProxyMiddleware({
  ...
  onProxyReq: (proxyReq, req, res) => {
    ...
    if (req.url?.startsWith('/api/') && !req.url?.includes('/api/auth/')) {
      ...
      if (token) {
        try {
          const payload = verifyToken(token.startsWith('Bearer ') ? token.substring(7) : token)
          if (payload) {
...
// Keep the middleware for now until proxy is fully implemented
